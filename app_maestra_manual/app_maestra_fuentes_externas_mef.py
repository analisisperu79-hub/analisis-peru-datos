...
# Ejemplo: ?serie=mef_deuda_publica_soles&embed=true
#
...
SERIES = {
    "mef_deuda_publica_soles": {
        ...
        "url_csv": (
            ...
            "datos/mef/deuda_publica_soles_mef.csv"
        ),
    ...
    # Plantilla para futuras series externas:
    # "identificador_unico": {
...
DEFAULT_SERIE = "mef_deuda_publica_soles"
...
def cargar_serie_externa(
    ...
    """
    Lee una serie externa desde un CSV de GitHub.

    La función recibe todos los datos relevantes como parámetros para que
    Streamlit mantenga un caché independiente para cada serie.
    ...
    # Bytes -> DataFrame. UTF-8-sig tolera archivos guardados con BOM.
    from io import StringIO
    contenido = r.content.decode("utf-8-sig")
    base = pd.read_csv(StringIO(contenido))

    if columna_periodo not in base.columns:
        raise ValueError(
            f"El CSV no contiene la columna de periodo '{columna_periodo}'."
        )

    if columna_valor not in base.columns:
        raise ValueError(
            f"El CSV no contiene la columna de valor '{columna_valor}'."
        )

    df = pd.DataFrame({
        "periodo_original": base[columna_periodo].astype(str).str.strip(),
        nombre_corto: pd.to_numeric(
            base[columna_valor],
            errors="coerce",
        ),
    })
...
try:
    ...
    # No representa necesariamente la fecha oficial de publicación de la fuente.
    ...
    st.error(
        f"No fue posible cargar la serie desde la fuente externa ({SERIE['fuente']})."
    )
...
if df_total.empty:
    st.error(
        ...
        "Revisa las columnas periodo/valor y su formato."
    )
    st.stop()
