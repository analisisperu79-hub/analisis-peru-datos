# ============================================================
# ANÁLISIS PERÚ — APP MAESTRA MULTIDIMENSIONAL
# ============================================================
# Reutilizable para bases con PERIODO + DIMENSIÓN + VALOR.
# Una sola app sirve múltiples bases mediante ?serie=CODIGO y un selector de dimensión.
#
# Flujo estándar:
# 1) Leer base multidimensional desde CSV alojado en GitHub
...
# Ejemplo: ?serie=inei_pbi_departamento&departamento=Tumbes&embed=true
#
# Para agregar una nueva serie externa, añade una entrada a SERIES y crea su CSV en GitHub.

SERIES = {
    # --------------------------------------------------------
    # INEI — PBI POR DEPARTAMENTO
    # --------------------------------------------------------
    "inei_pbi_departamento": {
        "nombre": "Producto Bruto Interno por departamento",
        "codigo": "inei_pbi_departamento",
        ...
        # Parámetro opcional para abrir una categoría directamente.
        # Ejemplo:
        # ?serie=inei_pbi_departamento&departamento=Tumbes
    ...
    # --------------------------------------------------------
    # PLANTILLA PARA FUTURAS BASES MULTIDIMENSIONALES
    # --------------------------------------------------------
    # "identificador_unico": {
    #     "nombre": "Nombre visible de la base",
    #     "codigo": "identificador_unico",
    #     "nombre_corto": "variable",
    #     "frecuencia": "M",  # M / Q / A
    #     "unidad": "Unidad de medida",
    #     "fuente": "INEI / MEF / SBS / SUNAT / MINCETUR / otra",
    #     "url_csv": "https://raw.githubusercontent.com/.../archivo.csv",
    #     "columna_periodo": "periodo",
    #     "columna_dimension": "categoria",
    #     "columna_valor": "valor",
    #     "etiqueta_dimension": "Categoría",
    #     "parametro_dimension": "categoria",
    #     "permitir_log": True,
    #     "permitir_ajuste_estacional": False,
    # },
}

DEFAULT_SERIE = "inei_pbi_departamento"
...
CODIGO_SERIE = str(_codigo_url).strip().lower()
...
# ============================================================
# 3. PARSEO DE PERIODOS DE FUENTES EXTERNAS
# ============================================================
# Formatos recomendados en el CSV:
#   Mensual:     2026-01
#   Trimestral:  2026Q1
#   Anual:       2026
#
# También se aceptan formatos como 2026mes1, 2026tri1 y fechas ISO.
...
def periodo_externo_a_fecha(etiqueta, frecuencia):
    if etiqueta is None or (isinstance(etiqueta, float) and np.isnan(etiqueta)):
        return pd.NaT
    ...
    # --------------------------------------------------------
    if freq == "A":
        m = re.search(r"(18\d{2}|19\d{2}|20\d{2}|21\d{2})", txt)
        return pd.Timestamp(int(m.group(1)), 1, 1) if m else pd.NaT

    # --------------------------------------------------------
    # TRIMESTRAL
    # Acepta: 2026Q1, 2026T1, 2026TRI1, 2026-1
    # --------------------------------------------------------
    if freq == "Q":
        m = re.search(
            r"(18\d{2}|19\d{2}|20\d{2}|21\d{2})\s*(?:[-_/ ]?\s*(?:Q|T|TRI)?\s*)([1-4])",
            txt,
        )
        if m:
        ...
        m = re.search(
            r"(?:Q|T|TRI)\s*([1-4])\D*(18\d{2}|19\d{2}|20\d{2}|21\d{2})",
            txt,
        )
        if m:
    ...
    # --------------------------------------------------------
    # MENSUAL
    # Acepta: 2026-01, 2026/01, 2026M1, 2026MES1
    # y nombres de meses.
    # --------------------------------------------------------
    if freq == "M":
        m = re.search(
            r"(18\d{2}|19\d{2}|20\d{2}|21\d{2})\s*(?:[-_/ ]|M|MES)*\s*(0?[1-9]|1[0-2])(?:\D|$)",
            txt,
        )
        if m:
            return pd.Timestamp(int(m.group(1)), int(m.group(2)), 1)

        ymatch = re.search(r"(18\d{2}|19\d{2}|20\d{2}|21\d{2})", txt)
        if ymatch:
        ...
        # Último intento: fecha ISO o formato reconocible por pandas.
        try:
            fecha = pd.to_datetime(txt, errors="raise")
...
# ============================================================
# 4. CARGA DE CSV EXTERNO DESDE GITHUB
# ============================================================

@st.cache_data(ttl=CACHE_TTL, show_spinner=False)
def cargar_base_multidimensional(
    url_csv,
    columna_periodo,
    columna_dimension,
    columna_valor,
    ...
    """
    Lee una base multidimensional desde un CSV alojado en GitHub.
    ...
    El nombre real de la dimensión puede ser departamento, actividad,
    ciudad, producto, sector, etc.
    """
    r = requests.get(
        url_csv,
        timeout=30,
    ...
    base = pd.read_csv(StringIO(contenido))
    ...
    df = pd.DataFrame({
        "periodo_original": base[columna_periodo].astype(str).str.strip(),
        "dimension": base[columna_dimension].astype(str).str.strip(),
        nombre_corto: pd.to_numeric(base[columna_valor], errors="coerce"),
    })

    # Elimina dimensiones vacías o textos nulos típicos.
    df["dimension"] = df["dimension"].replace(
        {"": np.nan, "nan": np.nan, "None": np.nan, "NaN": np.nan}
    )

    df["fecha"] = df["periodo_original"].apply(
        lambda x: periodo_externo_a_fecha(x, frecuencia)
    )

    df = (
        df.dropna(subset=["fecha", "dimension", nombre_corto])
          .sort_values(["dimension", "fecha"])
          .drop_duplicates(["dimension", "fecha"], keep="last")
...
# ============================================================
# 11. CARGA Y SELECCIÓN DE DIMENSIÓN
# ============================================================

try:
    df_total_base = cargar_base_multidimensional(
        SERIE["url_csv"],
        SERIE["columna_periodo"],
        SERIE["columna_dimension"],
        SERIE["columna_valor"],
        SERIE["nombre_corto"],
        FREQ,
    )

    consulta_fuente = datetime.now(
        ZoneInfo("America/Lima")
    )

except Exception as e:
    st.error(
        f"No fue posible cargar la base desde la fuente externa ({SERIE['fuente']})."
    )
    st.caption(f"Detalle técnico: {type(e).__name__}: {e}")
    st.stop()

if df_total_base.empty:
    st.error(
        "El archivo CSV no contiene observaciones utilizables. "
        "Revisa las columnas periodo, dimensión y valor."
    )
    st.stop()
...
# ------------------------------------------------------------
dimensiones = sorted(
    df_total_base["dimension"].dropna().astype(str).unique().tolist()
)

if not dimensiones:
    st.error("No se encontraron categorías válidas en la dimensión seleccionada.")
    st.stop()

parametro_dimension = SERIE.get("parametro_dimension", "categoria")
etiqueta_dimension = SERIE.get("etiqueta_dimension", "Categoría")

_dimension_url = st.query_params.get(parametro_dimension, None)
if isinstance(_dimension_url, list):
    _dimension_url = _dimension_url[0] if _dimension_url else None

# Intenta respetar la dimensión solicitada por URL sin distinguir mayúsculas.
dimension_inicial = dimensiones[0]
if _dimension_url:
    objetivo = str(_dimension_url).strip().casefold()
    coincidencia = next(
        (d for d in dimensiones if str(d).casefold() == objetivo),
        None,
    )
    if coincidencia is not None:
        dimension_inicial = coincidencia

indice_inicial = dimensiones.index(dimension_inicial)
...
# Mantener el parámetro en la URL facilita compartir una categoría concreta.
st.query_params["serie"] = CODIGO_SERIE
st.query_params[parametro_dimension] = dimension_seleccionada

df_total = (
    df_total_base.loc[
        df_total_base["dimension"] == dimension_seleccionada
    ]
    .sort_values("fecha")
    .reset_index(drop=True)
)
...
if df_total.empty:
    st.error(
        f"No hay observaciones disponibles para {etiqueta_dimension.lower()} "
        f"'{dimension_seleccionada}'."
    )
    st.stop()

# A partir de aquí, el resto de la app trabaja como una serie temporal normal.
nombre_base_original = SERIE["nombre"]
SERIE["nombre"] = f"{nombre_base_original} — {dimension_seleccionada}"

periodos = df_total["periodo"].tolist()
ultimo_periodo_disponible = periodos[-1]

st.caption(
    f"{etiqueta_dimension}: {dimension_seleccionada} · "
    f"Último periodo disponible: {ultimo_periodo_disponible} · "
    f"Fuente: {SERIE['fuente']} · "
    f"Última consulta al archivo: "
    f"{consulta_fuente.strftime('%d/%m/%Y %H:%M')} (hora de Perú)"
)
